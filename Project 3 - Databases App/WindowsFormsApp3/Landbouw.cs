...
namespace WindowsFormsApp3
{
    public partial class Landbouw : Form
    {
        ...
        private void Landbouw_Load(object sender, EventArgs e)
        {
        ...
        private void Recreatie_CheckedChanged(object sender, EventArgs e)
        {
            SqlConnection dbConnection = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=D:\Github\Project-3-Datasets\Project 3 - Databases App\WindowsFormsApp3\Database for project 3 - Databases.mdf;Integrated Security=True");
            dbConnection.Open();
            SqlCommand queryMisdaden = new SqlCommand(@"SELECT ROW_NUMBER() OVER(ORDER BY opp_rec desc, p.prov_naam), opp_rec, p.prov_naam
                                                                    FROM oppervlakte as o, provincie as p
                                                                    WHERE periode LIKE '2010%'
                                                                    AND p.prov_id = o.prov_id
                                                                    ORDER BY opp_rec DESC", dbConnection);
            SqlDataReader reader = queryMisdaden.ExecuteReader();
            DataTable table = new DataTable();
            table.Load(reader);
            LandGrid.DataSource = table;
        }
        ...
        private void Land_CheckedChanged(object sender, EventArgs e)
        {
            SqlConnection dbConnection = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=D:\Github\Project-3-Datasets\Project 3 - Databases App\WindowsFormsApp3\Database for project 3 - Databases.mdf;Integrated Security=True");
            dbConnection.Open();
            SqlCommand queryMisdaden = new SqlCommand(@"SELECT ROW_NUMBER() OVER(ORDER BY opp_lbv desc, p.prov_naam), opp_lbv, p.prov_naam
                                                                    FROM oppervlakte as o, provincie as p
                                                                    WHERE periode LIKE '2010%'
                                                                    AND p.prov_id = o.prov_id
